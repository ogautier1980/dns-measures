% Chapter 3 — auto-generated from chapitre3_methodologie.md
...
Notre système s'articule autour de \textbf{quatre composantes principales} inspirées de l'architecture OpenINTEL \cite{vanRijswijk2016} mais adaptées aux contraintes de RIPE Atlas :
...
\begin{tabular}{l l l}
\toprule
\textbf{Dimension} & \textbf{Choix} & \textbf{Raison} \\
\midrule
...
Nous utilisons la \textbf{liste Tranco} \cite{LePochat2019} plutôt que les listes commerciales (Alexa, Umbrella, Majestic, Quantcast) pour les raisons suivantes :
...
\textbf{Résistance à la manipulation} : Tranco requiert un effort de manipulation \textbf{4___P74_MATH___ supérieur} aux listes individuelles (Le Pochat et al., 2019, p. 2). Alexa peut être manipulé avec une seule requête HTTP pour entrer dans le top 1M, tandis que Tranco combine quatre sources et utilise une moyenne sur 30 jours, rendant la manipulation beaucoup plus coûteuse.
...
\toprule
\textbf{Option} & \textbf{Domaines} & \textbf{Mesures/jour} & \textbf{Coût crédits} & \textbf{Faisabilité} \\
\midrule
...
\begin{itemize}
  ...
  \item \textbf{Fréquence} : 1$\times$/jour
  \item \textbf{Total} : 10,000 domaines $\times$ 100 sondes $\times$ 1 query = \textbf{1,000,000 résultats/jour}
  \item \textbf{Sur 3 mois} : 1M $\times$ 90 jours = \textbf{90 millions de résultats}
\end{itemize}
...
\begin{itemize}
  ...
  \item Raison : Limiter interférence timing \cite{Holterbach2015}
...
\begin{itemize}
  ...
  \item Taux : 46.5\% sondes publiques dual-stack \cite{Nosyk2024}
...
\begin{tabular}{l l l l l}
\toprule
\textbf{Région} & \textbf{Sondes RIPE} & \textbf{\% Total} & \textbf{Sondes allouées} & \textbf{\% Allocation} \\
\midrule
...
Basé sur le tutorial Bortzmeyer et les best practices RIPE Atlas \cite{Nosyk2024}, nous configurons les mesures DNS comme suit :
...
\begin{lstlisting}[style=json]
{
  "definitions": [{
    ...
    "interval": 86400,                // 1x/jour (24h = 86400s)
...
\textbf{`use\_probe\_resolver: true`} : C'est le paramètre le plus critique. En utilisant le resolver local de chaque sonde, nous capturons la diversité géographique réelle des réponses DNS :
\begin{itemize}
  \item Sondes Europe $\rightarrow$ Resolvers ISP européens $\rightarrow$ Réponses CDN optimisées pour Europe
  \item Sondes Asie $\rightarrow$ Resolvers ISP asiatiques $\rightarrow$ Réponses CDN optimisées pour Asie
  \item Sans ce paramètre, toutes les sondes interrogeraient le même resolver $\rightarrow$ perte de diversité
\end{itemize}

\textbf{`set\_nsid\_bit: true`} : Permet d'identifier le serveur DNS autoritaire répondant via l'option NSID (RFC 5001). Utile pour :
\begin{itemize}
...
\textbf{`include\_abuf: true`} : Le champ \texttt{abuf} contient la réponse DNS complète encodée en base64. Essentiel pour :
\begin{itemize}
...
\textbf{`interval: 86400`} : Mesures quotidiennes (24h) permettent de :
\begin{itemize}
...
\textbf{`tags`} : Tags systématiques pour :
\begin{itemize}
...
\textbf{Problème} : Holterbach et al.~\cite{Holterbach2015} ont démontré deux types d'interférence sur RIPE Atlas :
\begin{enumerate}
  \item \textbf{Timing interference} : Augmentation latences (+1-7 ms selon hardware)
  \item \textbf{Scheduling interference} : Désynchronisation jusqu'à \textbf{1 heure}
\end{enumerate}
...
\begin{itemize}
  ...
  \item Mesures DNS = rapides (<100ms typique) $\rightarrow$ moins sensibles que traceroute
\end{itemize}

\textbf{Mitigation scheduling interference} :
\begin{itemize}
  \item \textbf{Ne pas se fier aux timestamps programmés} $\rightarrow$ Utiliser \texttt{timestamp} réels dans résultats
  \item Tolérance temporelle : Fenêtre de \textbf{___P86_MATH___2h} autour de l'horaire prévu
...
# Tolérance : $\pm$2h (acceptable: 22:00-02:00)
...
\begin{verbatim}
Crédits = base_cost x num_probes x num_measurements x duration_days

\end{verbatim}


Pour nos mesures DNS récurrentes (1$\times$/jour) :
\begin{itemize}
...
\begin{verbatim}
10,000 domaines x 10 crédits x 100 sondes x 90 jours / 10,000 (normalisation API)
...
\begin{itemize}
  ...
  \item Justification : Nosyk et al.~\cite{Nosyk2024} recommandent explicitement cette pratique
\end{itemize}
...
\begin{itemize}
  ...
  \item Exemple : \texttt{google.com}, \texttt{youtube.com}, \texttt{gmail.com} $\rightarrow$ même NS records
...
\begin{itemize}
  ...
  \item Si budget serré : réduire fréquence (1$\times$/2 jours) ou nombre sondes (100$\rightarrow$50)
...
\begin{tabular}{l l l}
\toprule
\textbf{Filtre} & \textbf{Taux exclusion} & \textbf{Justification} \\
\midrule
\textbf{Pas de résultat} & \textasciitilde{}1-2\% & Sondes offline, network issues \\
\textbf{RCODE  C0  NOERROR} & \textasciitilde{}2-3\% & NXDOMAIN, SERVFAIL (DNS errors) \\
...
\textbf{TOTAL} & \textbf{\textasciitilde{}10-15\%} & \textbf{Résultats exploitables : 85-90\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item Compression efficace : \textbf{Ratio 1:7.4} \cite{vanRijswijk2016}
...
\toprule
\textbf{Format} & \textbf{Taille/jour} & \textbf{Taille 90j} & \textbf{Compression} \\
\midrule
...
\begin{itemize}
  \item \textbf{\% domaines SINGLE\_IP} (pas de diversité géographique)
  \item \textbf{\% domaines HIGH\_DIVERSITY} (forte diversité géographique)
  \item \textbf{Corrélation géographie ___P91_MATH___ IP} (CDN vs anycast)
...
\textbf{Hypothèse} : Corrélation négative attendue (TTL court $\rightarrow$ changements fréquents).
...
\begin{itemize}
  ...
  \item \textbf{Corrélation TTL ___P93_MATH___ changement} (coefficient Spearman + p-value)
...
\textbf{Hypothèse} : Si Asie/Afrique/Am. Sud apportent >10\% IPs uniques malgré <10\% sondes $\rightarrow$ biais significatif.
...
\textbf{Hypothèse} : Resolvers ECS devraient avoir Jaccard similarity >0.8 avec ISP local \cite{Wang2018}.
...
\begin{itemize}
  \item Fréquence mesures : 1$\times$/jour maximum (vs horaire ou minute)
...
\begin{itemize}
  ...
  \item Pas de corrélation sondes $\leftrightarrow$ utilisateurs individuels
...
\textbf{Technique} :
\begin{itemize}
  \item [OK] Collecte $\geq$85\% résultats valides (filtrage <15\%)
  \item [OK] Couverture $\geq$95\% domaines Tranco Top 10K
  \item [OK] Distribution géographique $\geq$4 continents
