...
Q4 --- \textit{Resolver impact}: Do the three major public \ac{DNS} resolvers --- Google (8.8.8.8), Cloudflare (1.1.1.1), and Quad9 (9.9.9.9) --- return consistent \ac{DNS} responses for the same domain queried from the same geographic location? How large is the cross-resolver divergence, and which infrastructure factors drive it?

Sixty-six days, one measurement per day: 2~April through 6~June~2026. A pilot phase (02--09~April, 99~domains, 50~probes) confirmed the fetch-and-parse pipeline before the main campaign scaled to 250~domains $\times$ 200~probes on 11~April. Each morning a cron job fetched the previous day's results from the RIPE Atlas \ac{API} and parsed them into dated Parquet files on a Raspberry Pi at home, synced nightly to Google Drive. The Q4 one-off campaign (750~measurements, 250~domains $\times$ 3~resolvers $\times$ 200~probes) ran in June 2026. All four questions draw from the same corpus and probe set.
