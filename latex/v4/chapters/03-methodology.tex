...
To address Q4, a supplementary one-off measurement campaign is conducted on the full 250-domain corpus using the same 200 probes as Q1--Q3. Using the same probe set ensures that any observed inter-resolver divergence is attributable to resolver infrastructure rather than geographic probe differences. The campaign provides three simultaneous resolver observations per (domain, probe) pair:
...
The Q4 campaign uses \texttt{set\_rd\_bit: true} and a \ac{UDP} payload of 4096 bytes, appropriate for recursive resolver queries. Measurements are configured as one-off (\texttt{is\_oneoff: true}) to avoid continuous credit consumption. A fourth configuration targeting the probe's local \ac{ISP} resolver via \texttt{use\_probe\_resolver: true} was planned but abandoned: the RIPE Atlas \ac{API} rejects the combination of a target \ac{IP} field and \texttt{use\_probe\_resolver: true}, and probe-side inspection showed that a large fraction of RIPE Atlas probes resolve their \texttt{/etc/resolv.conf} to loopback addresses or directly to public resolvers, making the \ac{ISP} condition structurally indistinguishable.

The three configurations test whether resolver infrastructure (anycast routing, \ac{ECS} policy) changes the \ac{CDN}-assigned \ac{IP} addresses a probe receives for the same domain.

\subsection{Credit Budget}
\label{subsec:3.3.4}

Periodic measurements cost 10 credits per probe per instance; one-off measurements cost 20 credits per probe (results delivered immediately). The Q4 one-off campaign costs $750 \times 200 \times 20 = 3{,}000{,}000$ credits, spread over three days due to RIPE Atlas's rolling 24-hour credit limit of 1,000,000 credits.
...
Main --- primary (direct auth)   & 250 & 200 & 57 days & $\approx$28,500,000 \\
Main --- Q4 (3 resolvers, once)  & 250 & 200 & once    & $\approx$3,000,000 \\
\textbf{Total campaign}          &     &     &         & $\approx$\textbf{31,950,000} \\
\bottomrule
\end{tabular}
\end{table}

The estimated total of approximately 31,950,000 credits exceeds the initial balance of 29,000,000; the difference was covered by credits accumulated from hosting two RIPE Atlas probes throughout the campaign. The credit budget is monitored by the log output of \texttt{create\_ripe\_measurements.py}, which prints the estimated daily and total consumption before any measurements are created.
...
Q4 measures how much resolver infrastructure changes the answer a probe receives. The one-off campaign (Section~\ref{subsec:3.3.3}) runs the same 200 probes against each of the 250 domains three times --- once per public resolver (\texttt{google}, \texttt{cloudflare}, \texttt{quad9}).

Each resolver is first profiled independently: error rate (non-zero RCODE fraction), median \ac{RTT}, and mean answer count. These baselines establish whether any resolver is structurally noisier or slower before the cross-resolver comparison begins.

The core comparison is pairwise across all three resolver pairs: Google vs.\ Cloudflare, Google vs.\ Quad9, Cloudflare vs.\ Quad9. For each (domain, probe) pair, the \texttt{answer\_ips} returned by each resolver are compared; a divergence event is recorded when the two \ac{IP} sets are completely disjoint (no shared address). The per-pair divergence rate is the fraction of (domain, probe) tuples showing divergence.

Reported figures: error rate and median \ac{RTT} per resolver; pairwise \ac{IP} divergence rates for all three resolver pairs.
