...
The Q4 campaign reveals that resolver choice is not a neutral variable in \ac{DNS} measurement: querying Google 8.8.8.8, Cloudflare 1.1.1.1, or Quad9 9.9.9.9 from the same probe for the same domain produces completely different \ac{IP} sets in 52--55\% of observations. This rate is consistent across all three resolver pairs, pointing to a structural property of the ecosystem.

Two mechanisms explain it. \textit{Anycast infrastructure}: each resolver operates an independent global anycast network. From a given probe, the path to 8.8.8.8, 1.1.1.1, and 9.9.9.9 terminates at different physical servers with different upstream \ac{BGP} contexts. Those servers forward queries from their own \ac{IP} space; \ac{CDN} authoritative servers interpret that address as the geographic signal and return a location-optimised answer pool accordingly. Resolver selection therefore acts as a second geographic variable, layered on top of the probe's actual location.

\textit{\ac{ECS} policy}: Cloudflare propagates the client's full \ac{IP} prefix; Google truncates to /24; Quad9 suppresses \ac{ECS} entirely~\cite{RFC7871}. \ac{CDN} operators implementing \ac{ECS}-aware routing~\cite{Wang2018} respond to those different signals with different \ac{IP} pools for the same domain, even when the underlying query comes from the same client.

The practical implication for the Q1--Q3 methodology is direct: by querying authoritative servers with the \ac{RD} bit cleared, Q1--Q3 bypasses both mechanisms entirely. The probe's own \ac{IP} determines the \ac{CDN} answer, and no resolver anycast or \ac{ECS} artefact is introduced. The 50\%+ divergence rate measured in Q4 quantifies exactly what Q1--Q3 avoids --- and confirms that the design choice was correct.
...
Executing the Q4 one-off campaign required navigating two RIPE Atlas constraints. The 1,000,000-credit rolling 24-hour limit forced the 750 measurements to be spread across three separate days. The global limit of 25 concurrent one-off measurements to the same target \ac{IP} (shared across all RIPE Atlas users) required automatic retry logic in the creation script. Both constraints were worked around programmatically; the campaign completed in full.
...
Public resolver divergence (Q4) & Wang~\cite{Wang2018}; Hours~\cite{Hours2016} & \textbf{Confirmed}: 52--55\% cross-resolver \ac{IP} divergence; Cloudflare median \ac{RTT} 37\% lower than Google/Quad9 \\
\bottomrule
\end{tabularx}
...
\textit{ISP resolver comparison.}
The Q4 design originally included a fourth configuration targeting each probe's local \ac{ISP} resolver. This condition was abandoned after discovering that the RIPE Atlas \ac{API} rejects the combination of a target \ac{IP} field and \texttt{use\_probe\_resolver: true}, and that a large fraction of RIPE Atlas probes resolve their \texttt{/etc/resolv.conf} to loopback addresses or directly to a public resolver, making the \ac{ISP} condition structurally indistinguishable. A future study comparing \ac{ISP} recursive resolvers would require either platform-level support for capturing the actual resolver identity per probe, or a custom measurement agent able to record the resolver used for each query.
...
Q4 --- \textit{Resolver impact.} \textbf{52--55\%} cross-resolver \ac{IP} divergence for all three resolver pairs: the same probe querying the same domain through Google, Cloudflare, or Quad9 gets completely different \ac{IP} addresses in more than half of cases. Cloudflare's median \ac{RTT} (15.5\,ms) is 37\% lower than Google (24.7\,ms) and Quad9 (23.4\,ms). Divergence is driven by independent anycast routing and divergent \ac{ECS} policies. Resolver choice is a confound of comparable magnitude to probe location.
